...
from .collator import LLaDASFTCollator,NormalDLLMSFTCollator,ARMSFTCollator
...
from ..utils.template import modelType2Template
# 在 main 函数的开始部分
...
def main():
    ...
    template = modelType2Template['dllm'](tokenizer)
    tokenizer.chat_template = template.get_chat_template()
    ...
    generation_config_dict = template.get_generation_config()
    generation_config = GenerationConfig.from_dict(generation_config_dict)
    model.generation_config = generation_config
    model.generation_config = template.generation_config



    if args.mode == 'llama':
        collator = NormalDLLMSFTCollator(template, max_length=args.max_length)
    elif args.mode == 'llada':
        collator = LLaDASFTCollator(template,max_length=args.max_length)
    else:
        collator = NormalDLLMSFTCollator(
            tokenizer, 
    ...
    training_args = TrainingArguments(
        ...
        lr_scheduler_kwargs={'num_warmup_steps': 0}, # midtrain的学习率直接衰减到0就好 
        save_strategy="steps",
