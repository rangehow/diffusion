#!/bin/bash
# =========================================================================
#  Submit reasoning task experiments to FMG queue (h800_fmg_exp)
#
...
#    bash script/sh/submit_reasoning.sh                                   # 110m only
#    bash script/sh/submit_reasoning.sh --size 400m                       # 400m only
#    bash script/sh/submit_reasoning.sh --size both                       # 110m + 400m
#    bash script/sh/submit_reasoning.sh --dry-run                         # preview
#    bash script/sh/submit_reasoning.sh --force                           # resubmit all
#    bash script/sh/submit_reasoning.sh --task cd3 --mode reasoning_card  # single job
# =========================================================================
...
echo "============================================="
echo "  Reasoning Tasks → FMG Queue (h800_fmg_exp)"
echo "  Experiment: fmg_h800_ci"
echo "============================================="
...
HOPE_TEMPLATE="hope/reasoning_fmg_1node.hope"
...
for MODEL_SIZE in "${SIZES[@]}"; do
    ...
    #   CD3: cutoff_len=37, 600 epochs (diffusion), 40 epochs (AR)
    #   CD4: cutoff_len=64, 600 epochs (diffusion), 40 epochs (AR)
    #   CD5: cutoff_len=74, 600 epochs (diffusion), 40 epochs (AR)
    # We use same epochs for all modes (600) for fair comparison
