#!/bin/bash
# =========================================================================
#  Submit reasoning task experiments — 2 NODES (16 GPUs)
#
#  Supports both FMG and M17 queues via --queue flag.
#
#  Follows Ye et al. (2024) "Beyond Autoregression" setting.
#  2x faster training than 1-node version.
...
#    bash script/sh/submit_reasoning_2node.sh                              # FMG (default)
#    bash script/sh/submit_reasoning_2node.sh --queue m17                  # M17
#    bash script/sh/submit_reasoning_2node.sh --queue m17 --dry-run        # preview
#    bash script/sh/submit_reasoning_2node.sh --size 400m                  # 400m only
#    bash script/sh/submit_reasoning_2node.sh --size both                  # 110m + 400m
#    bash script/sh/submit_reasoning_2node.sh --force                      # resubmit all
#    bash script/sh/submit_reasoning_2node.sh --task cd3 --mode reasoning_card  # single
# =========================================================================
...
if [ "${QUEUE}" = "fmg" ]; then
    ...
    HOPE_TEMPLATE_NAME="reasoning_fmg_2node.hope"
elif [ "${QUEUE}" = "m17" ]; then
    EXPERIMENT_FLAG="syn_fmg_h800"
    HOPE_TEMPLATE_NAME="reasoning_m17_2node.hope"
else
    echo "ERROR: --queue must be 'fmg' or 'm17'"
    exit 1
fi

echo "============================================="
echo "  Reasoning Tasks → ${QUEUE^^} Queue (2 nodes × 8 GPUs)"
...
for MODEL_SIZE in "${SIZES[@]}"; do
    ...
    # Batch size per device for 2-node (16 GPUs total)
    # 110M model on 141GB H800: use large BS for GPU utilization
    # eff_batch = 16 * BS * grad_accum (grad_accum auto-computed in train script)
    # LR scaled by sqrt(eff_batch / 1024) per linear scaling rule
    if [ "${MODEL_SIZE}" = "400m" ]; then
        ...
        # 16 GPUs * 128 = eff_batch 2048 (2x baseline) → LR * sqrt(2) ≈ 4.2e-4
        CD_BS=128
        SUDOKU_BS=64
        CD_LR="4.2e-4"
        SUDOKU_LR="4.2e-4"
    ...
    #   CD3: cutoff_len=37, 600 epochs
    #   CD4: cutoff_len=64, 600 epochs
    #   CD5: cutoff_len=74, 600 epochs
    submit_job cd3 reasoning_card  600 ${CD_BS}     37  ${CD_LR} ${MODEL_SIZE}
    submit_job cd3 reasoning_ar    600 ${CD_BS}     37  ${CD_LR} ${MODEL_SIZE}
    submit_job cd3 reasoning_mdlm  600 ${CD_BS}     37  ${CD_LR} ${MODEL_SIZE}

    submit_job cd4 reasoning_card  600 ${CD_BS}     64  ${CD_LR} ${MODEL_SIZE}
    submit_job cd4 reasoning_ar    600 ${CD_BS}     64  ${CD_LR} ${MODEL_SIZE}
    submit_job cd4 reasoning_mdlm  600 ${CD_BS}     64  ${CD_LR} ${MODEL_SIZE}

    submit_job cd5 reasoning_card  600 ${CD_BS}     74  ${CD_LR} ${MODEL_SIZE}
    submit_job cd5 reasoning_ar    600 ${CD_BS}     74  ${CD_LR} ${MODEL_SIZE}
    submit_job cd5 reasoning_mdlm  600 ${CD_BS}     74  ${CD_LR} ${MODEL_SIZE}

    # Sudoku: cutoff_len=165 (BOS+81puzzle+SEP+81solution+EOS), 300 epochs
    submit_job sudoku reasoning_card  300 ${SUDOKU_BS}  165 ${SUDOKU_LR} ${MODEL_SIZE}
    submit_job sudoku reasoning_ar    300 ${SUDOKU_BS}  165 ${SUDOKU_LR} ${MODEL_SIZE}
    submit_job sudoku reasoning_mdlm  300 ${SUDOKU_BS}  165 ${SUDOKU_LR} ${MODEL_SIZE}
done
