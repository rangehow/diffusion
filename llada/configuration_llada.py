...
class LLaDAConfig(PretrainedConfig):
    ...
    def __init__(self, use_cache: bool = False, **kwargs):
        ...
        # NOTE (transformers >= 5.3 compat): We handle weight tying ourselves
        # in LLaDAModelLM (via forward() using F.linear(x, wte.weight)).
        # Set tie_word_embeddings=False so HF's get_expanded_tied_weights_keys()
        # returns {} and doesn't try to traverse non-existent ff_out module.
        all_kwargs.setdefault("tie_word_embeddings", False)
        super().__init__(**all_kwargs)
        # Explicitly set attributes that newer transformers (>=4.57) no longer provides as defaults
