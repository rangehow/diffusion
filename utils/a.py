...
def sh_filtered_finefineweb(local_path):
    """加载 sh_filtered_finefineweb 数据集 (带文本规范化)"""
    print(f"从本地路径加载数据集 'sh_filtered_finefineweb': {local_path}")
    dataset = datasets.load_from_disk(local_path)
    
    # 应用文本规范化
    dataset = dataset.map(
        _create_sh_normalizer_batch(),
        batched=True,
        num_proc=64,
        desc="Normalizing sh_filtered_finefineweb"
    )
    return dataset
...
def sh_fineweb_edu_10b(local_path):
    ...
    dataset = dataset.map(
        ...
        batched=True,
        num_proc=64,
        desc="Normalizing sh_fineweb_edu_10b"
    )
    return dataset
...
def sh_fineweb_edu_1b(local_path):
    ...
    dataset = dataset.map(
        ...
        batched=True,
        num_proc=64,
        desc="Normalizing sh_fineweb_edu_1b"
    )
    return dataset
...
def sh_lm1b_test(local_path):
    ...
    # 应用文本规范化 (detokenization + truecasing)
    dataset = dataset.map(
        _create_sh_normalizer_batch(),
        batched=True,
        num_proc=64,
        desc="Normalizing sh_lm1b_test"
    )
    return dataset
...
def sh_openwebtext(local_path):
    ...
    dataset = dataset.map(
        ...
        batched=True,
        num_proc=64,
        desc="Normalizing sh_openwebtext"
    )
    return dataset
