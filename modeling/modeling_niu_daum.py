...
def flash_attention_forward(
    ...
    **_kwargs,
    ...
    if position_ids is not None:
        # V3 reordering: apply RoPE using custom position_ids instead of cu_seqlens-based
        # sequential positions. This allows tokens to be physically reordered while
        # maintaining correct relative position encoding.
        #
        # position_ids: (total_nnz,) — the logical position of each physical token
        # cos_cache, sin_cache: (max_pos, rotary_dim/2)
        rotary_emb._update_cos_sin_cache(max_seqlen, device=qkv.device, dtype=qkv.dtype)
        cos = rotary_emb._cos_cached[position_ids]  # (total_nnz, rotary_dim/2)
        sin = rotary_emb._sin_cached[position_ids]  # (total_nnz, rotary_dim/2)
        # Apply to Q and K (first 2 of the 3 in QKV)
        # qkv shape: (total_nnz, 3, nheads, headdim)
        rotary_dim = cos.shape[-1] * 2
        qk = qkv[:, :2, :, :rotary_dim]  # (total_nnz, 2, nheads, rotary_dim)
        # Reshape cos/sin for broadcasting: (total_nnz, 1, 1, rotary_dim/2)
        cos = cos[:, None, None, :]  # (total_nnz, 1, 1, rotary_dim/2)
        sin = sin[:, None, None, :]  # (total_nnz, 1, 1, rotary_dim/2)
        half_dim = rotary_dim // 2
        qk_r = qk[..., :half_dim] * cos - qk[..., half_dim:] * sin
        qk_i = qk[..., :half_dim] * sin + qk[..., half_dim:] * cos
        qkv[:, :2, :, :rotary_dim] = torch.cat([qk_r, qk_i], dim=-1)
    else:
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if indices is None and cu_seqlens is None and max_seqlen is None:
                ...
                if inputs_embeds is None:
                    with torch.no_grad():
                        input_ids, indices, cu_seqlens, max_seqlen, position_ids, _, _ = _unpad_modernbert_input(
                            inputs=input_ids, attention_mask=attention_mask, position_ids=position_ids
                        )
                else:
                    inputs_embeds, indices, cu_seqlens, max_seqlen, position_ids, _, _ = _unpad_modernbert_input(
...
def _unpad_modernbert_input(
    ...
    daum_weights = daum_weights.flatten()[indices] if daum_weights is not None else None

    return unpadded_inputs, indices, cu_seqlens, max_seqlen_in_batch, unpadded_position_ids, unpadded_labels,daum_weights
...
def ForCausalLMLoss(
    ...
    daum_weights: Optional[torch.Tensor] = None,
    **kwargs,
):
    logits = logits.float()
    ...
    unweighted_total_loss = torch.tensor(0.0, device=logits.device)
    if normalizer > 0:
        # 2. 基于原始 loss 计算未加权的总 loss (用于打印)
        unweighted_total_loss = ((per_token_loss * valid_token_mask).sum() / normalizer).detach()


    loss_for_backprop = per_token_loss
    if daum_weights is not None:
        loss_for_backprop = loss_for_backprop * daum_weights

    # Mask for tokens that were originally [MASK]
    is_masked_token_mask = (input_ids == mask_token_id)

    # Combine masks to get our final groups
    masked_loss_mask = valid_token_mask & is_masked_token_mask
    non_masked_loss_mask = valid_token_mask & ~is_masked_token_mask

    # 4. Calculate the sum of losses for each component by applying the masks.
    # Multiplying by the boolean mask zeros out the losses for tokens not in the group.
    if normalizer > 0:
        total_masked_loss_sum = (loss_for_backprop * masked_loss_mask).sum() / normalizer
        total_non_masked_loss_sum = (loss_for_backprop * non_masked_loss_mask).sum() / normalizer
        total_loss = total_masked_loss_sum + total_non_masked_loss_sum
    else:
        total_masked_loss_sum = torch.tensor(0.0, device=logits.device)
        total_non_masked_loss_sum = torch.tensor(0.0, device=logits.device)
...
def calculate_daum_weights(
    ...
    Empirically 1.9× faster at S=8192 and 4.8× at S=16384 on CPU.
    """
...
class ModernBertForDiffusionLM(ModernBertPreTrainedModel,GenerationMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        daum_weights = kwargs.get('daum_weights', None)
        if daum_weights is None and use_daum:
            daum_weights = calculate_daum_weights(input_ids,mask_token_id)
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if indices is None and cu_seqlens is None and max_seqlen is None:
                ...
                if inputs_embeds is None:
                    with torch.no_grad():
                        input_ids, indices, cu_seqlens, max_seqlen, position_ids, labels,daum_weights = _unpad_modernbert_input(
                            inputs=input_ids, attention_mask=attention_mask, position_ids=position_ids, labels=labels,daum_weights = daum_weights
                        )
                else:
                    inputs_embeds, indices, cu_seqlens, max_seqlen, position_ids, labels,daum_weights = _unpad_modernbert_input(
                        inputs=inputs_embeds, attention_mask=attention_mask, position_ids=position_ids, labels=labels,daum_weights = daum_weights
                    )
        else:
        ...
        if labels is not None:
            lm_loss,masked_lm_loss,non_masked_lm_loss,unweighted_total_loss = ForCausalLMLoss(logits,labels,input_ids=input_ids,mask_token_id = mask_token_id,vocab_size=self.config.vocab_size,daum_weights=daum_weights,**kwargs)
        
        # print(lm_loss.device,lm_loss,masked_lm_loss,non_masked_lm_loss,unweighted_total_loss,current_mlm_prob)
        # 后处理，还原去pad的序列
