...
class ModernBertPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module: nn.Module):
        ...
        if isinstance(module, ModernBertEmbeddings):
            ...
            init_weight(module.mlm_classifier,stds["out"])
...
def TokenSigmoidFocalLoss(
    inputs: torch.Tensor,
    targets: torch.Tensor,
    gamma: float = 2.0,
    alpha: Optional[float] = None, # 允许传入固定的alpha，或者动态计算
    reduction: str = "mean", # 添加reduction参数，更灵活
):
    """
    适用于Token级别二分类任务的动态加权Focal Loss。
    ...
    """
    ...
    # 动态计算 alpha
    if alpha is None:
        num_positives = torch.sum(targets == 1).float()
        num_negatives = torch.sum(targets == 0).float()
        total = num_positives + num_negatives

        # 即使没有正样本，alpha_t的计算依然有效，只是 dynamic_alpha 会变成 1.0
        # 在这种情况下，负样本的权重 (1-alpha) 会是 0，这可能不是我们想要的。
        # 一个更稳健的alpha计算方式是给正负样本数量都加上平滑项
        # alpha = (num_negatives + 1) / (total + 2)
        # 这里我们先用原始的方式，但要注意这个问题。
        if total == 0: # 如果batch为空
            return torch.tensor(0.0, device=inputs.device)

        dynamic_alpha = (num_negatives + 1) / (total + 2)
    else:
        dynamic_alpha = alpha

    ce_loss = F.binary_cross_entropy_with_logits(inputs, targets.float(), reduction="none")
    prob = inputs.sigmoid()
    p_t = prob * targets.float() + (1 - prob) * (1 - targets.float())
    modulating_factor = (1 - p_t) ** gamma

    alpha_t = dynamic_alpha * targets.float() + (1 - dynamic_alpha) * (1 - targets.float())

    loss = alpha_t * modulating_factor * ce_loss

    # 归一化 (Reduction)
    if reduction == "mean":
        # 按照Focal Loss论文的建议，由正样本的数量来归一化
        num_positives = torch.sum(targets == 1).float()
        return loss.sum() / max(1.0, num_positives.item())
    elif reduction == "sum":
        return loss.sum()
    else: # "none"
        return loss
...
class ModernBertForDiffusionLM(ModernBertPreTrainedModel,GenerationMixin):
    _tied_weights_keys = {"mlm_classifier.weight": "model.embeddings.tok_embeddings.weight"}
    ...
    def __init__(self, config):
        ...
        self.mlm_classifier = nn.Linear(config.hidden_size, config.vocab_size, bias=getattr(config, "decoder_bias", False))
        self.mlm_loss_weight = getattr(config, "mlm_loss_weight", 1.0)

        # 2. [条件性] Token Change Prediction 任务组件
        self.token_change_classifier = None
        if getattr(self.config, "use_token_change_task", False):
            logger.info("Token Change Prediction task is enabled.")
            self.head = ModernBertPredictionHead(config)
            self.token_change_classifier = nn.Linear(config.hidden_size, 1)
            self.token_change_loss_weight = getattr(config, "token_change_loss_weight", 1.0)
        else:
            logger.info("Token Change Prediction task is disabled.")
    ...
    def forward(
        ...
        **kwargs,
        ...
        # --- 获取 mask_token_id ---
        # 确保能从config中获取到mask_token_id，这在HuggingFace模型中是标准做法
        # 你的 CausalLMCollator 已经使用了 tokenizer.mask_token_id，
        # 确保模型config中也存储了这个值
        ...
        mlm_logits = (
            ...
            if self.config.reference_compile
            else self.mlm_classifier(last_hidden_state)
        )


        if self.config.use_token_change_task:
        ...
        if labels is not None:
            ...
            if current_mlm_prob is not None:
                ...
                t = current_mlm_prob.to(mlm_logits.device)
                weights_per_sentence = 1.0 / (t + 1e-8)

                # 根据是否使用 Flash Attention 决定如何扩展权重
                # Flash Attention 模式下，输入是 unpadded 的
                is_masked = (input_ids == mask_token_id)
                if self.config._attn_implementation == "flash_attention_2" and cu_seqlens is not None:
                    # cu_seqlens 的形状是 (batch_size + 1,)
                    # 我们需要根据它来构建一个映射，将每个 token 对应到它的句子索引
                    
                    seq_lens = cu_seqlens[1:] - cu_seqlens[:-1]

                    # 创建句子索引，例如 [0, 0, 0, 1, 1, 2, 2, 2, 2, ...]
                    sentence_indices = torch.arange(
                        batch_size, device=mlm_logits.device, dtype=torch.long
                    ).repeat_interleave(seq_lens)

                    mlm_weights = weights_per_sentence[sentence_indices]
                    ntp_weights = torch.ones_like(mlm_weights)
                    per_token_mlm_weights = torch.where(is_masked, mlm_weights, ntp_weights)
                # 标准模式下，输入是 padded 的 (B, S, H)
                else:

                    mlm_weights = weights_per_sentence.unsqueeze(1)
                    

                    ntp_weights = torch.ones_like(mlm_weights) # Shape (B, 1), 会被广播

                    per_token_mlm_weights = torch.where(is_masked, mlm_weights, ntp_weights)

            
            mlm_loss = ForMaskedLMLoss(mlm_logits, labels, vocab_size=self.config.vocab_size,per_token_weights=per_token_mlm_weights,**kwargs)
            # mlm_loss1 = ForMaskedLMLoss(mlm_logits, labels, vocab_size=self.config.vocab_size)
        
            # shifted_logits = mlm_logits[:-1, :].contiguous()
            # shifted_labels = labels[ 1:].contiguous()
            # mlm_loss = ForMaskedLMLoss(shifted_logits, shifted_labels, vocab_size=self.config.vocab_size)

        if self.config.use_token_change_task and token_change_labels is not None:
            
            # 根据是否使用 flash attention 来决定如何计算 loss
            if self.config._attn_implementation == "flash_attention_2":
                # Flash Attention 模式下，logits 和 labels 已经 unpad
                flat_logits = token_change_logits.view(-1)
                flat_labels = token_change_labels.view(-1)
                ...
                if active_logits.numel() > 0:
                    token_change_loss = TokenSigmoidFocalLoss(active_logits, active_labels)
                else:
                    token_change_loss = torch.tensor(0.0, device=token_change_logits.device)

            else: # 标准 Attention 模式
                # 将 logits 和 labels 展平
                if token_change_logits.dim() == 3:
                    flat_logits = token_change_logits.squeeze(-1).view(-1)
                else:
                    flat_logits = token_change_logits.view(-1)
                flat_labels = token_change_labels.view(-1)

                active_mask = (attention_mask.view(-1) == 1) & (flat_labels != -100)
        ...
        if labels is not None and self.config.use_token_change_task and token_change_labels is not None:
        ...
        return MLMAndTokenChangeOutput(
            loss=total_loss,
            mlm_loss=mlm_loss,
            token_change_loss=token_change_loss if self.config.use_token_change_task else None,
            logits=mlm_logits,
            token_change_logits=token_change_logits if self.config.use_token_change_task else None,
